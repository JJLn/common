...
@implementation JJAlertItem
@end

//private
@interface JJAlert()<UIActionSheetDelegate>{
...
- (NSInteger)addButtonWithTitle:(NSString *)title{
    ...
    JJAlertItem *item = [[JJAlertItem alloc] init];
    item.title = title;
    item.action = ^(JJAlertItem *item) {
        NSLog(@"no action");
    };
    item.type = ITEM_OK;
...
- (NSString *)buttonTitleAtIndex:(NSInteger)buttonIndex{
    JJAlertItem *item = _items[buttonIndex];
    return item.title;
}
- (NSArray *)actions
{
...
-(void)show8
{
    ...
    for (JJAlertItem *item in _items)
    {
...
//        }
//        UIAlertController *alertView = [UIAlertController alertControllerWithTitle:_title message:_message preferredStyle:style];
//        
//        return;
//    }
    if (_styleType == STYLE_ALERT) {
...
+ (void)showMessage:(NSString *)title message:(NSString *)message
{
    ...
    JJAlert *alert = [[JJAlert alloc]initWithTitle:title andMessage:message style:STYLE_ALERT];
